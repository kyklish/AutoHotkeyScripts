...
Edit(sFilePath, oLogFile)
{
    ...
    ;DAYTIME ===============================================================
    ;TODO
    ; Можно сравнительно легко продлить день (замедлив течение времени) и укоротить ночь (ускорив течение времени), отредактировав в файле "initial.cache_block" (из архива "initial.pak") параметры:
    ; timeSettingsDay = 0.1 (по умолчанию 1)
    ; timeSettingsNight = 10 (по умолчанию 1)
    ; Данные параметры в файле есть в двух местах. Редактировать надо везде.
    ; Я бы порекомендовал следить за тем, чтобы не менялось общее количество символов (и, соответственно, размер файла).
    ; То-есть, чтобы последняя цифра отредактированного значения была на том же месте, что и значение "1" в оригинальном файле.
    ; Но это только моё предположение, которое я не проверял (лень!).
