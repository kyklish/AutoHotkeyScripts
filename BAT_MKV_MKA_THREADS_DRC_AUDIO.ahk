...
GetThreadCmd() {
    ...
    sCmd .= "ping -n 2 localhost > NUL`n"
    sCmd .= "FOR %%F IN (""*AUDIO*_DRC*_THREAD*.DONE"") DO (`n"
    sCmd .= "    SET /a NumDone+=1`n"
    sCmd .= "    DEL /F /Q ""%%F""`n"
    sCmd .= ")`n"
...
GetStaxRipCmd(sStaxRipTemplate) {
    ...
    sCmd .= "`n"
    sCmd .= "StaxRip -NoFocus -ClearJobs ""-LoadTemplate:%Template%"" ""-AddBatchJobs:%FileList%"" -StartJobs -Exit`n"
    sCmd .= "`n"
    sCmd .= "CALL :BELL`n"
    sCmd .= "CALL :BELL`n"
    sCmd .= "CALL :BELL`n"
...
GetStaxRipDelOrigCmd(sStaxRipTemplate) {
    ...
    sCmd .= "    StaxRip -NoFocus -ClearJobs ""-LoadTemplate:%Template%"" ""-AddBatchJob:%%F"" -StartJobs -Exit`n"
    sCmd .= "    DEL /F ""%%F""`n"
    sCmd .= "    ECHO DONE: ""%%F""`n"
    sCmd .= "    CALL :BELL`n"
    sCmd .= ")`n"
