	...
	if (A_Args.Length() == 0) {
		...
		if (A_Args.Length() == 1) {
			...
			if (A_IsAdmin) {
				...
				MsgBox, Must be run with admin rights`nA_Args = %sParam%
			}
			ExitApp
		} else {
			MsgBox, Too many parameters were passed to script
			ExitApp
		}
	}
}
...
HotKey_StartFan()
{
	...
	Run_WaitScriptAsAdmin(A_ScriptFullPath, "Start")
}
...
HotKey_StopFan()
{
	; TrayIcon_Functions work only in user mode!!! with admin rights it's hangs Explorer.exe!!!
	TrayIcon_Button("speedfan.exe", "L", true) ; double click tray icon of SpeedFan
	Run_WaitScriptAsAdmin(A_ScriptFullPath, "Stop")
}