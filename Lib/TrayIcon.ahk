...
TrayIcon_GetInfo(sExeName := "")
{
    ...
    oTrayInfo := []
    For key,sTray in ["Shell_TrayWnd", "NotifyIconOverflowWindow", "TrayNotifyWnd", "SysPager", "ToolbarWindow32", "TopLevelWindowForOverflowXamlIsland"]
    {
        ...
        SendMessage, 0x0418, 0, 0, TrayNotifyWnd%idxTB%, ahk_class %sTray%
        Loop, %ErrorLevel%
        {
             ; TB_GETBUTTON 0x0417
            SendMessage, 0x0417, A_Index-1, pRB, TrayNotifyWnd%idxTB%, ahk_class %sTray%
...
; ..............: sTray - Place where to find the icon ("Shell_TrayWnd" or "NotifyIconOverflowWindow" or "TopLevelWindowForOverflowXamlIsland").
...
TrayIcon_GetTrayBar(sTray:="Shell_TrayWnd")
{
    ...
    RegExMatch(ControlList, "(?<=TrayNotifyWnd)\d+(?!.*TrayNotifyWnd)", nTB)
    Loop, %nTB%
    {
        ControlGet, hWnd, hWnd,, TrayNotifyWnd%A_Index%, ahk_class %sTray%
