...
GetCredentials(bAdmin)
{
	...
	;Только встроенная учетка позволяет запускать от имени админа в AutoHotkey
...
;В качестве логина админа нужна встроенная учетная запись админа
;Только встроенная учетка позволяет запускать приложения от имени админа в AutoHotkey
