...
;Даже никогда не смотрю вниз какая раскладка включена. Легче быстро тапнуть по нужному Shift.
...
return
;-------------------------------------------------------------------------------------
;Предположил нажимаете LControl (левый шифт) сразу в переменную startLControl заносится временная метка.
;Следующая команда Input last_key, L1 V, {RControl} ждет нажатия клавиши клавиатуры (если вы нажимаете шифт чтобы ввести прописную букву, а не переключить язык).
;Далее, когда отпускаете шифт срабатывает код после ~LControl UP:: - снимается временная метка и проверяется, а сколько времени вы держали шифт нажатым?
;Если недолго и при этом last_key ничему не равно, т.е. во время нажатия и удержания шифта никакая другая клавиша не была нажата, то надо переключить раскладку.
