...
ToggleCheckbox(name)
{
	; ControlClick, %name%, , , Left, 1, NA ; кликаем мышкой или чекаем непостредственно командой ниже
	Control, Check, , %name% ;UnCheck не работает для SpeedFan!!! только Check
	if (ErrorLevel)
		MsgBox, CheckBox - Can't toggle "%name%".
	return ErrorLevel
}
...
CheckVisibleControl(name, ByRef isVisible)
{
	...
	if (ErrorLevel) {
		MsgBox, ControlGet - No such input field "%name%".
		return ErrorLevel
	}
}
...
FindVisibleControl(name, ByRef index)
{
	...
	;return valut = non zero - didn't find or error
}
;-------------------------------------------------------------------------------------
#IfWinExist, SpeedFan
+^F5:: ;CPU_Fan_On
