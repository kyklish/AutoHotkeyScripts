...
return
;-------------------------------------------------------------------------------------
;Предположил нажимаете LShift (левый шифт) сразу в переменную startLShift заносится временная метка.
;Следующая команда Input last_key, L1 V, {RShift} ждет нажатия клавиши клавиатуры (если вы нажимаете шифт чтобы ввести прописную букву, а не переключить язык).
;Далее, когда отпускаете шифт срабатывает код после ~LShift UP:: - снимается временная метка и проверяется, а сколько времени вы держали шифт нажатым?
