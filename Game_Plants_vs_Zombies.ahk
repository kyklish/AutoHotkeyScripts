...
-::ToggleAutoClick()
=::ToggleNumberOnPlant()
q:: ClickOnColor(0xFEFE48) ; Gold Coin - Very difficult find out Color
...
1-9 -> Карточки растений в верхнем ряду.
  ...
  `` -> Выкопать цветок
  - -> Вкл\Откл авто клика по солнышкам и монеткам (ложные срабатывания в меню).
  = -> Вкл\Откл подсказки с номером иконки растения.
  q -> Попытаться авто-кликнуть золотую монетку (сложно определить код цвета).
)")
...
FindSlotsCoordinates()
{
	; в начале игры дают 6ть слотов для растений + лопата, после увеличивают до 7и и 8и + лопата
	; при добавлении 7го слота увеличивается длина рамки вокруг иконок растений, чтобы уместить 7й слот (расстояние между иконками не меняется)
	; при добавлении 8го слота размер рамки не меняется, уменьшается расстояние между иконками растений
	; проверять только по длине рамки или только по расстоянию между иконками не получится, нужна комбинация этих условий
	; 6ть слотов определяем по правой границе рамки, граница рамки на месте, значит это 6ть слотов
	; 7мь слотов проверяем по пикселю перед 7ой иконкой растения, если это цвет фона, значит у нас 7мь слотов (если нет, значит иконки стоят плотнее и нужна проверка на 8мь слотов)
	; 8мь слотов аналогично определяем по пикселю перед 8ой иконкой растения
	global Xcard, slots
	backgroundColor := 0x6E3213 ; цвет фона, на котором располагаются иконки растений
...
CalculateXcard(slots)
{
	...
	; 8 слотов растений = шаг в 54 пикселя
	offset := []
...
ToggleNumberOnPlant()
{
	...
	if (toggle := !toggle) {
		for i, X in Xcard {
			; по умолчанию используется ToolTip №1 (уже занят), начинаем сразу со второго
			; ширина ToolTip с одной цифрой 18 пикселей (-9 чтобы поставить по центру карточки растения)
			if (isDebug)
				ToolTip, % i . " Debug", % X, 0, % i + 1 ; для отладки, левая сторона ToolTip укажет X координату клика мышки при посадке растения
			else
				ToolTip, % i, % X - 9, 0, % i + 1
			if (i == slots + 1)
				break
